...
def main():
    ...
    combined_df.to_csv(gene_expression_file, sep='\t')
...
if __name__ == "__main__":
    ...
    for opt, arg in opts:
        ...
        if opt in ("--expression_file"):
            ...
            expression_file = '/input_data/README_glass_expression.txt'
        # glass clinical file with subtypes / mutations
        if opt in ("--clinical_file"):
            clinical_file = str(arg)
            clinical_file = '/input_data/README_glass_clinical.txt'

        # HGNC-annotated file describing protein-coding genes
        if opt in ("--protein_coding_file"):
            protein_coding_file = str(arg)
            protein_coding_file = '/input_data/hgnc_gene_annotations.tsv'

        # results DGEA file
        if opt in ("--gene_expression_file"):
            gene_expression_file = str(arg)
            gene_expression_file = '/intermediate_files/glass_expression.tsv'
        if opt in ("--dgea_file"):
            dgea_file = str(arg)
            dgea_file = '/intermediate_files/glass_dgea.tsv'
