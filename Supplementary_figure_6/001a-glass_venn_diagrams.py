...
if __name__ == "__main__":
    ...
    for opt, arg in opts:
        ...
        if opt in ("--combined_pval_file"):
            ...
            combined_pval_file = '/input_data/intermediate_files/glass_pvals.tsv'
        # file with fold changes for each gene from each method
        if opt in ("--combined_fc_file"):
            combined_fc_file = str(arg)
            combined_fc_file = '/input_data/intermediate_files/glass_fcs.tsv'

        # figure script
        if opt in ("--figure_script"):
            figure_script = str(arg)
            figure_script = '~/001b-glass_venn_diagrams.R'
        # file to generate figure
        if opt in ("--figure_data_file"):
            figure_data_file = str(arg)
            figure_data_file = '/input_data/intermediate_files/glass_venn_diagrams.tsv'
        # file to save figure
        if opt in ("--figure_file"):
            figure_file = str(arg)
            figure_file = '/output_data/glass_venn_diagrams.pdf'
